...
public class MovieDemo extends JPanel
        implements ActionListener, ListSelectionListener {
    private static final long serialVersionUID = 1L;
    JTable table = null;
    JTextField edits[] = new JTextField[5];

    adminMovieMgr mgr = new adminMovieMgr();
    ...
    void MovieTableInit() {
        mgr.readAll("movie.txt");

        final String[] columnNames = {"영화코드번호", "제목", "연령제한", "런닝타임", "가격"};
        DefaultTableModel tableModel = new DefaultTableModel(columnNames, 0);
        for (adminMovie a : adminMovieMgr.movieList)
